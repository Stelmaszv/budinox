<?php
	...
	class log_out extends sesion{
 	...
 	class sesion{
 		...
 		function try_login($login,$password){
 			if(!$this->sql->checksql('SELECT * FROM `b.admin` where login like "'.mysql_real_escape_string($login).'" and password like "'.mysql_real_escape_string($password).'" ')){
 	...
 	class sql{ 
 			...
 			function __construct($data=false){
 				$this->data=$data;
 				new db_conect("mysql.cba.pl","stelmaszv","stel_php_cba_pl","Samlogan007");
 			}
  ...
  class mesage_action{
     ...
     public function  reeded(){
         $icon=$this->messages->return_reed($this->messInfo->mesages_info['id'],'glyphicon glyphicon-ok','glyphicon glyphicon-remove');
          $this->menu.='<spam onclick="delete_mesage()" id="action_icon"  class="'.$icon.'" ></spam>';
     }
     public function delete_mesage(){
         $this->menu.= '<spam onclick="delete_mesage()" id="action_icon"  class="glyphicon glyphicon-trash" ></spam>';
     }
     public function mesage_anser(){
          $this->menu.= "<spam id='anser'><spam onclick=annser_mesage(".$this->messInfo->mesages_info['id'].",'".$this->messInfo->mesages_info['subject']."','on') id=action_icon  class='glyphicon glyphicon-envelope' ></spam></spam>";
     }
  }
  ...
  class message_sign_as_reed extends delate_message{
      function __construct($id,$confirm){
          $this->messInfo = new mesage_load($id);
          $this->id=$id;
          $this->confirm=$confirm;
          $this->sql= new sql();
          $this->manipulation();
          $this->messages = new messages($id);
      }
      private function manipulation(){
        if($this->confirm=='sign_mesage_as_reed'){
  ...
  class delate_message{
      ...
      private function manipulation(){
        if($this->confirm=='delete_mesage'){
          return $this->sql->MsQuery('DELETE FROM `b.mesages` WHERE `b.mesages`.`id` = '.intval($this->id).'');
        }
      }
      function step(){
        return $this->messages->load_mesages();
      }
  }
  ...
  class upload_photo{
 	...
 	class ajax{
      ...
      $objects["sign_mesage_as_reed"]= new message_sign_as_reed($this->array_sent['id_mesage'],$this->ajax);
 			$objects["sent"] 	 = new ajax_sent_emial($this->array_sent);	
      $objects["load_messag"]= new mesage_load($this->array_sent['id_mesage']);
      $objects["delete_mesage"]= new delate_message($this->array_sent['id_mesage'],$this->ajax);
